...
一个基于 **LangChain 1.0、阿里百炼 qwen3.5-plus、Tavily、SQLite 菜品图谱 RAG、FastAPI 与阿里云 OSS** 构建的多模态 AI 私厨 Agent。

上传冰箱或厨房里的食材照片，Agent 会自动识别食材，优先检索本地菜品图谱，并在需要时联网搜索菜谱，并从营养价值和制作难度两个维度进行评分排序，最终生成结构化的食谱建议；也支持直接输入食材清单进行多轮对话。

> 项目参考《黑马程序员agent项目实战》实现，API Key、阿里云 OSS 配置均使用环境变量管理。

## 项目定位

这是一个面向学习与二次开发的多模态 AI Agent Demo，不是单纯调用大模型的聊天页面，而是完整串联图片上传与 OSS 直传、qwen3.5-plus 食材识别、SQLite 菜品图谱 RAG、Tavily 联网补充、营养与难度评分排序，以及隐私优先的长期口味记忆。

## 工程亮点

- **本地优先 RAG**：先查 8 道演示菜品组成的 SQLite 图谱，结果不足时才请求 Tavily。
- **轻量向量存储**：384 维 float32 向量直接存 SQLite，不依赖外部 Embedding API；后续可迁移到 sqlite-vec 或 PostgreSQL + pgvector。
- **双层记忆设计**：短期会话 Checkpoint 与长期口味记忆分离，避免会话数据污染用户偏好。
- **隐私优先**：长期记忆采用 consent-first、目的限定和数据最小化，并支持导出、撤回和彻底删除。
- **工程可运行**：API Key 缺失时服务仍能启动；OSS 缺失时提供本地上传降级，便于先跑通页面和接口。
- **前后端一体交付**：FastAPI 直接托管响应式聊天页，无需额外前端构建步骤。
...
2. 优先调用 `search_dish_knowledge` 检索本地菜品图谱，本地结果不足时再调用 `web_search`。
